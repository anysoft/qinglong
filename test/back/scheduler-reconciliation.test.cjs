...
function fixture(client) {
  ...
  new Function('module', 'isDemoEnv', 'cronClient', 'withSchedulerMutation', 'TaskResourceResolver', js)(
    module,
    () => false,
    client,
    (fn) => fn(),
    class { async resolve(ids) { return ids.map(id=>({task:{id,enabled:true},readiness:{status:'READY'}})); } },
  );
