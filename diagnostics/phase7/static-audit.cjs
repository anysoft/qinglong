...
for (const file of files) {
  ...
  for (const [index, line] of source.split('\n').entries()) {
    ...
    if (!classification && /runtime|pyenvProvider|pythonEnvironment|pythonDependency/i.test(file) && /managed/i.test(line)) classification=['Platform-owned Runtime path; not Subscription mode','Runtime Manager','PLATFORM_CORE','KEEP'];
...
const report = {environment_files:environmentFiles.length,dependency_references:dependencyFindings,runtime_files:runtimeFiles.length,protected_phase5_files:protectedFiles, status: failures.length ? 'FAIL' : 'PASS', production_files: files.length,
