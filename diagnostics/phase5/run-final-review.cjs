...
try {
 ...
 for(const base of ['HEAD','develop']){
 const r=spawnSync('gitnexus',['detect-changes','--scope','compare','--base-ref',base,'--repo','qinglong','--limit','500'],{env,encoding:'utf8',maxBuffer:32*1024*1024});
 fs.writeFileSync(path.join(__dirname,`final-review-${base}.json`),JSON.stringify({base_ref:base,exit_code:r.status,report:r.stdout,stderr:r.stderr},null,2)+'\n');process.stdout.write(base+': '+(r.stdout+r.stderr).split('\n').slice(0,9).join('\n')+'\n');if(r.status)process.exitCode=r.status;
 }
