...
for (const file of files) {
  ...
  for (const [index, line] of source.split('\n').entries()) {
    ...
    if (!classification && file==='shell/preload/task_types.py')classification=['Logical Task DTO; old Cron DTO remains internal IPC only','Python Task types','PLATFORM_CORE','KEEP'];
    if (!classification && /^back\/services\/(?:task|schedulerBridge)/.test(file)) classification=['Task logical resources or explicit temporary adapter; no runtime activation','Task domain / B01 bridge','B01/B02/B03/B17','10'];
...
const protectedFiles=['back/services/configMaterialization.ts','back/services/dependence.ts','shell/task.sh','shell/otask.sh','shell/process_group.py','shell/hook_process.py'];
...
for(const file of files)for(const [index,line] of fs.readFileSync(path.join(root,file),'utf8').split('\n').entries()){
 ...
 if(!classification&&file==='back/services/taskResourceResolver.ts')classification='TASK_RESOURCE_METADATA: comment forbids materialized paths';
...
// Phase 9: all remaining Cron identifiers are classified, with domain deny gates.
const taskFiles=['back/data/task.ts','back/services/task.ts','back/services/taskResourceResolver.ts','back/services/taskResourceValidator.ts','back/shared/taskDefinition.ts'];
const taskViolations=/\b(?:Crontab|CronService|cron_id|sub_id)\b|(?:executable|venv|node_modules|build_id|absolute_cwd)\s*:/;
for(const file of taskFiles)for(const [index,line]of fs.readFileSync(path.join(root,file),'utf8').split('\n').entries())if(taskViolations.test(line)&&!(file==='back/services/task.ts'&&line.includes('draft.sub_id')))failures.push({file,line:index+1,reason:'Task domain uses bridge identity or execution snapshot'});
for(const file of ['src/pages/crontab/index.tsx','src/pages/crontab/modal.tsx','src/pages/crontab/detail.tsx'])if(fs.existsSync(path.join(root,file)))failures.push({file,reason:'Raw-command Task UI remains'});
const cronReferences=[];
for(const file of files)for(const [index,line]of fs.readFileSync(path.join(root,file),'utf8').split('\n').entries())if(/Crontab|CronService|\bcron_id\b|\bsub_id\b/.test(line)){
 let classification;
 if(file.startsWith('back/schema/')||file==='back/shared/operationalSchema.ts')classification='FROZEN_SCHEMA_OR_EXPLICIT_V5_MIGRATION';
 else if(file.startsWith('back/protos/')||file==='back/services/grpc.ts')classification='SCHEDULER_PROTOBUF_OR_RESERVED_TRANSPORT';
 else if(file==='back/services/task.ts'&&line.includes('draft.sub_id'))classification='B07_EXISTING_DISCOVERY_PLAN_INPUT';
 else if(/^back\/(?:data\/(?:cron|runningInstance)|services\/(?:cron|subscription|taskExecutionSourceBridge|taskExecutionPreparation|taskWorkspace|managedSubscription|script|subscriptionDiscovery)|api\/(?:cron|dashboard)|loaders\/(?:db|bootAfter|initData)|schedule\/|shared\/(?:runCron|pLimit|interface))/.test(file))classification='B03_B04_B07_B08_B14_TEMPORARY_PROJECTION_OR_TRANSPORT';
 else if(file==='back/validation/schedule.ts')classification='B03_B04_EXISTING_SCHEDULER_PROJECTION_VALIDATION';
 else if(file==='src/utils/index.ts'||file.startsWith('src/components/task-bridge/')||file.startsWith('src/pages/dashboard/')||file.startsWith('shell/'))classification='SCHEDULE_EXPRESSION_OR_B08_B14_RESULT_LOG_BRIDGE';
 if(!classification)failures.push({file,line:index+1,text:line.trim(),reason:'Unclassified Cron identifier'});
 cronReferences.push({file,line:index+1,text:line.trim(),classification});
}
const report = {cron_references:cronReferences,task_domain_files:taskFiles,node_files:nodeFiles.length,node_references:nodeFindings,environment_files:environmentFiles.length,dependency_references:dependencyFindings,runtime_files:runtimeFiles.length,protected_phase5_files:protectedFiles, status: failures.length ? 'FAIL' : 'PASS', production_files: files.length,
