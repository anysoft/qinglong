...
(async()=>{
 ...
 async function start(){backend=spawn(process.execPath,['--require',path.join(root,'tests/phase6/browser-provider.cjs'),path.join(root,'static/build/app.js')],{cwd:root,env,detached:true,stdio:['ignore','pipe','pipe']});backend.stdout.on('data',b=>output+=b);backend.stderr.on('data',b=>output+=b);await until(async()=>{const r=await fetch(base+'/api/system');return(await r.json()).code===200;});}
 ...
 await delay(300);assert.deepEqual(leaks,[]);evidence.response_payloads_inspected=inspected.length;evidence.websocket_frames_inspected=websocketFrames;assert.ok(websocketFrames>0,'real WebSocket frames must be inspected');mark('browser-secret-response-audit');evidence.runtime_provider='DETERMINISTIC_FIXTURE';evidence.status='PASS';mark('fresh-v3-fk-layout');for(const suffix of ['png','txt'])fs.rmSync(path.join(__dirname,'browser-failure.'+suffix),{force:true});
