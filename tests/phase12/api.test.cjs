...
test('Panel workspace routes reject Open access, unknown fields, absent concurrency tokens and unsafe error output',async t=>{
 ...
 assert.equal(require('node:fs').existsSync('back/api/script.ts'),false);
 for(const method of ['GET','PUT','POST','DELETE'])assert.equal((await fetch(base+'/api/scripts/detail',{method})).status,404);
});
