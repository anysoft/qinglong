...
const {attach}=require('./attach.cjs'),{task,wait}=require('./helpers.cjs'),{wheelhouse}=require('../phase7/wheelhouse.cjs');
const manifest=require('../../diagnostics/phase10/managed-runtime/result.json');
test('official managed Python imports from pinned venv across retry and Build promotion, without host dependency paths',async t=>{
 ...
 await require('./managed-entrypoints.cjs')(h,f.definition);
 ...
 await fs.writeFile('diagnostics/phase10/managed-python-execution.json',JSON.stringify({status:'PASS',runtime:runtime.version,pinned_build:first,promoted_build:second,real_venv_import:true,scheduled:true,timeout:true,cancel:true,retry_snapshot:true,host_path_excluded:true},null,2));
});
