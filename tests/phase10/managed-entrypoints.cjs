...
module.exports=async function parity(h,definition){
 ...
 const id=await h.load('back/shared/runCron.ts').runCron('ignored old command',{id:String(definition.id)});let result=await wait(h,id,30000);assert.equal(result.status,'SUCCESS',JSON.stringify(result));assert.equal(result.trigger_type,'SCHEDULE');assert.ok((await h.execution.log(id)).includes('SCHEDULED_MANAGED_HOOK'));
