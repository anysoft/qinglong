...
for(const language of ['sh','js','py'])test(`real ${language} current MAIN bridge, config in all phases, generated Secret patch/redaction and cleanup`,{timeout:30000},async t=>{
 const h=await runtime(t),file=`main.${language}`,task=await h.CrontabModel.create({command:`task ${file}`});
...
test('real BEFORE timeout kills descendants, skips remaining BEFORE/MAIN, runs AFTER_FAILURE and FINALLY',{timeout:30000},async t=>{
 const h=await runtime(t),task=await h.CrontabModel.create({command:'task main.sh'});await fs.writeFile(path.join(h.config.scriptPath,'main.sh'),'echo MUST_NOT_RUN');
...
for(const scenario of [
 ...
 const h=await runtime(t),task=await h.CrontabModel.create({command:'task main.sh'});await fs.writeFile(path.join(h.config.scriptPath,'main.sh'),scenario.main);
...
for(const tee of [false,true])test(`graceful cancellation tee=${tee} executes AFTER_FAILURE/FINALLY with config, then restores original`,{timeout:20000},async t=>{
 const h=await runtime(t),task=await h.CrontabModel.create({command:'task main.sh'}),target=path.join(h.config.scriptPath,'config.yaml');await fs.writeFile(target,'ORIGINAL');await fs.writeFile(path.join(h.config.scriptPath,'main.sh'),'echo ready > ready; sleep 20');
...
test('live execution pins Config revision, ENV and Hook plan across edits; next run observes new values',{timeout:25000},async t=>{
 const h=await runtime(t),task=await h.CrontabModel.create({command:'task main.sh'});await h.EnvModel.create({name:'SNAPSHOT_ENV',value:'ENV_OLD'});
...
test('all publication sources share the existing global publication-1 lease',{timeout:15000},async t=>{
 const h=await runtime(t);await fs.mkdir(path.join(h.config.scriptPath,'subscription-42'));await fs.writeFile(path.join(h.config.scriptPath,'subscription-42/main.sh'),'echo PUBLICATION_OK');const task=await h.CrontabModel.create({command:'task subscription-42/main.sh'});await fs.mkdir(path.join(h.root,'.locks'));
