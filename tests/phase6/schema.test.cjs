...
test('frozen v2 migrates to identical fresh v4, preserves ENV/Config/Hooks and enforces runtime CHECK/FK constraints',async t=>{
 ...
 assert.equal((await b.db.query('SELECT platform_schema_version FROM PlatformMetadata',{type:QueryTypes.SELECT}))[0].platform_schema_version,4);
