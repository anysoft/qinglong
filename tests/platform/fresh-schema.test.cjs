...
async function fixture(t) {
  ...
  const models = [];
  for (const file of ['gitCredential','repository','worktree','scopedEnv','subscription','cron','env','dependence','open','system','cronView','cronStats','runningInstance']) {
...
test('empty data root creates v1, all bridge/core tables, and restarts without schema writes',async t=>{
  ...
  assert.equal(metadata.platform_schema_version,1);
  await initializeOperationalSchema(database,models);
...
test('unknown nonempty database fails closed and preserves rows and schema',async t=>{
  ...
  const before=await schema(database);
  await assert.rejects(initializeOperationalSchema(database,models),{code:'UNSUPPORTED_DATABASE_SCHEMA',message:'UNSUPPORTED_DATABASE_SCHEMA: This platform requires a fresh database.'});
  assert.deepEqual(await schema(database),before);
...
test('partial DDL failure rolls back all objects and allows a clean retry',async t=>{
  const {database,models}=await fixture(t);
  const bad=database.define('Broken',{field:DataTypes.STRING});
  const original=bad.sync;
  bad.sync=async()=>{throw new Error('INJECTED_DDL_FAILURE')};
  await assert.rejects(initializeOperationalSchema(database,[...models,bad]),/INJECTED_DDL_FAILURE/);
