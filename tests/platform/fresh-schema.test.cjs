...
async function fixture(t) {
  ...
  const models = [];
  for (const file of ['gitCredential','repository','worktree','scopedEnv','subscription','cron','env','dependence','open','system','cronView','cronStats','runningInstance','configAsset','runtime','pythonEnvironment']) {
...
test('empty data root creates v4, all bridge/core tables, and restarts without schema writes',async t=>{
  ...
  assert.equal(metadata.platform_schema_version,4);
  await initializeOperationalSchema(database,models);
