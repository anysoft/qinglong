...
test('remote spelling changes preserve identity, storage and subscription references', async (t) => {
  const x = await setup(t);
  ...
  await x.storage.changeRemote(x.repo.id, x.repo.remote_url);
  assert.equal((await x.storage.get(x.repo.id)).storage_path, initialized.storage_path);
  assert.equal(await x.SubscriptionModel.count({ where: { repository_id: x.repo.id } }), 1);
});
