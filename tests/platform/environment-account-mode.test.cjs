...
for (const language of ['sh', 'js', 'py']) test(`scoped ${language} account concurrency keeps split secrets in private temporary logs`, async t => {
  ...
  const output = await new Promise((resolve, reject) => {
    const cp = spawn(process.execPath, [path.resolve('tests/phase5/snapshot-main.cjs'), `accounts.${language}`, 'conc', 'ACCOUNTS'], { env: { ...process.env, QL_DIR: h.dir, QL_DATA_DIR: path.join(h.dir, 'data'), real_time: 'true', ID: '', QL_TASK_ENV_SNAPSHOT: snapshot.directory } });
