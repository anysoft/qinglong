...
test('v4 → v5 preserves real Python environment references; fresh matches migration and late failure rolls back',async t=>{
 ...
 await h.initializeOperationalSchema(h.db,h.models);const migrated=(await h.db.query('SELECT * FROM PlatformMetadata',{type:QueryTypes.SELECT}))[0];assert.equal(migrated.platform_schema_version,8);
