...
async function attach(root,index){
 ...
 for(const name of ['gitCredential','repository','worktree','scopedEnv','subscription','cron','env','dependence','open','system','cronView','cronStats','runningInstance','configAsset','runtime','pythonEnvironment']){const m=load(path.resolve('back/data/'+name+'.ts'),mocks,cache);Object.assign(all,m);mocks['../data/'+name]=m;}
