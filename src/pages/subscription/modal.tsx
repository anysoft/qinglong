...
const SubscriptionModal = ({ subscription, handleCancel }: { subscription?: any; handleCancel: (updated?: any) => void }) => {
  ...
  return <Modal title={intl.get(subscription ? '编辑订阅' : '创建订阅')} open centered maskClosable={false} onOk={() => handleOk().catch(() => {})} onCancel={() => handleCancel()} confirmLoading={loading}>
    <Form form={form} layout="vertical" initialValues={{ schedule_type: 'crontab', ...subscription, is_disabled: !!subscription?.is_disabled, autoAddCron: subscription?.autoAddCron == null || !!subscription.autoAddCron, autoDelCron: subscription?.autoDelCron == null || !!subscription.autoDelCron }}>
      ...
      {([['whitelist', 'Include'], ['blacklist', 'Exclude'], ['dependences', '辅助文件筛选'], ['extensions', '文件后缀（空格分隔）']] as const).map(([name, label]) => <Form.Item key={name} name={name} label={label}><Input /></Form.Item>)}
