...
const SubscriptionModal = ({ subscription, handleCancel }: { subscription?: any; handleCancel: (updated?: any) => void }) => {
  ...
  return <Modal title={intl.get(subscription ? '编辑订阅' : '创建订阅')} open centered maskClosable={false} onOk={() => handleOk().catch(() => {})} onCancel={() => handleCancel()} confirmLoading={loading}>
    <Form form={form} layout="vertical" initialValues={{ schedule_type: 'crontab', ...subscription, is_disabled: !!subscription?.is_disabled, autoAddCron: subscription?.autoAddCron == null || !!subscription.autoAddCron, autoDelCron: subscription?.autoDelCron == null || !!subscription.autoDelCron }}>
      ...
      {([['whitelist', 'Include'], ['blacklist', 'Exclude'], ['dependences', '辅助文件筛选'], ['extensions', '文件后缀（空格分隔）'], ['sub_before', '执行前命令'], ['sub_after', '执行后命令']] as const).map(([name, label]) => <Form.Item key={name} name={name} label={label}><Input /></Form.Item>)}
      <Form.Item name="autoAddCron" label={intl.get('自动添加任务')} valuePropName="checked"><Switch /></Form.Item>
      <Form.Item name="autoDelCron" label={intl.get('自动删除任务')} valuePropName="checked"><Switch /></Form.Item>
      <Form.Item name="is_disabled" label="禁用" valuePropName="checked"><Switch /></Form.Item>
    </Form>
    {subscription?.id && <><Button loading={loading} onClick={async () => { setLoading(true); try { const r = await request.post(`${config.apiPrefix}subscriptions/${subscription.id}/prepare`, {}); if (r.code === 200) message.success(`Worktree #${r.data.worktree_id} 已准备`); } finally { setLoading(false); } }}>准备已保存的订阅</Button>{subscription.worktree_id && <Alert message={`Worktree #${subscription.worktree_id} · ${subscription.last_sync_state || '未同步'}`} />}<TaskEnvironment id={subscription.id} subscription /></>}
  </Modal>;
};
