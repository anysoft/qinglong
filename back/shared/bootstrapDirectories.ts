...
/** Backend-owned directories, including the current execution and dependency bridges. */
export async function bootstrapDirectories(dataRoot: string): Promise<void> {
  ...
  for (const name of ['db', 'config', 'scripts', 'deps', 'dep_cache', 'log', 'syslog', 'bak', 'upload', 'git', 'worktrees', '.locks', 'tmp']) {
