// Frozen Phase 10 schema. Never regenerate from later models.
export default {
  "source_commit": "e1ee6f8023a9c090131d3ae51c67aa4eedf73e4c",
  "metadata": {
    "platform_schema_version": 7,
    "model_signature": "ee4c749817e5ecf76008cc82ca26a33b8aeae3665d4c8fe369d77e54466ce5f2",
    "schema_signature": "a3bebfe86f7c285aef4f9a09e4b09aca6f77b5a640760bb3a4fee1643f1f2174"
  },
  "objects": [
    {
    ...
    },
    {
      ...
      "sql": "CREATE TABLE PlatformMetadata (platform_schema_version INTEGER NOT NULL PRIMARY KEY CHECK(platform_schema_version = 7), model_signature TEXT NOT NULL, schema_signature TEXT NOT NULL)"
    },
    {
    ...
    },
    {
      ...
      "sql": "CREATE TRIGGER task_run_fields_insert BEFORE INSERT ON TaskRuns WHEN NEW.attempt_count<0 OR NEW.cancel_requested NOT IN (0,1) OR NEW.trigger_type NOT IN ('MANUAL','SCHEDULE','API','INTERNAL') OR NEW.concurrency_policy NOT IN ('FORBID','QUEUE','ALLOW') OR (NEW.task_id IS NULL AND NEW.status IN ('QUEUED','RESOLVING','RUNNING')) BEGIN SELECT RAISE(ABORT,'TASK_RUN_FIELDS_INVALID'); END"
    },
    {
      "name": "task_run_fields_update",
      "sql": "CREATE TRIGGER task_run_fields_update BEFORE UPDATE ON TaskRuns WHEN NEW.attempt_count<0 OR NEW.cancel_requested NOT IN (0,1) OR NEW.trigger_type NOT IN ('MANUAL','SCHEDULE','API','INTERNAL') OR NEW.concurrency_policy NOT IN ('FORBID','QUEUE','ALLOW') OR (NEW.task_id IS NULL AND NEW.status IN ('QUEUED','RESOLVING','RUNNING')) BEGIN SELECT RAISE(ABORT,'TASK_RUN_FIELDS_INVALID'); END"
    },
    {
